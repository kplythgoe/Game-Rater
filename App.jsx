...
export default function App() {
  ...
  return (
    <>
      ...
      <div className="boxes">
        <Box>
          {search.length > 0 ? <p>Showing results for: {search}</p> : <p>Searched game results will show here</p>}
    ...
    <header>
      <div className="container">
        <div className="header-wrapper">
          <div className='title'><h1>GameLog</h1></div>
...
function SingleGame({game, setCurGame}) {
  return (
  <div className="single-game" key={game.name} onClick={(e) => setCurGame(game)}>
  ...
  return (
    <div className='game-log-summary'>
      ...
      <div className='game-log-averages'>
        <div className='game-log-user-av'><p>🎮 Your average rating: X</p></div>
      </div>
    </div>
  )
}
    ...
    <div className='current-game'>
      ...
      <div className="information-rating-section">
        <div className='current-game-information'>
          ...
          <p>🎮 Platforms: {curGame.platforms.map(platform => platform.platform.name).join(", ")}</p>
          <p>⚔️ Genre: {curGame.genres.map(genre => genre.name).join(", ")}</p>
